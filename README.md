...
- **Domains** (`plm_data/domains/`) — Domain creation and metadata are registry-backed. Each domain has its own subpackage with a `spec.py` that declares dimension, parameter ranges, boundary names/roles, allowed boundary-condition families, allowed initial-condition families, and periodic boundary pairs. The current repo ships `interval`, `rectangle`, `box`, `disk`, `dumbbell`, `parallelogram`, `channel_obstacle`, `y_bifurcation`, `venturi_channel`, `porous_channel`, `serpentine_channel`, `l_shape`, `multi_hole_plate`, `airfoil_channel`, `side_cavity_channel`, and `annulus`. The Gmsh-backed domains require `python-gmsh`; `plm_data/core/mesh.py` keeps legacy registrations and mesh-facing compatibility imports during the migration.

...
- **Domains** (`plm_data/domains/`) — Domain creation and metadata are registry-backed. Each domain has its own subpackage with a `spec.py` that declares dimension, parameter ranges, boundary names/roles, allowed boundary-condition families, allowed initial-condition families, and periodic boundary pairs, plus a `gmsh.py` that builds the tagged Gmsh model. The current repo ships `interval`, `rectangle`, `box`, `disk`, `dumbbell`, `parallelogram`, `channel_obstacle`, `y_bifurcation`, `venturi_channel`, `porous_channel`, `serpentine_channel`, `l_shape`, `multi_hole_plate`, `airfoil_channel`, `side_cavity_channel`, and `annulus`. All domains are Gmsh-backed and require `python-gmsh`; `plm_data/core/mesh.py` only keeps mesh-facing compatibility imports.
- **Boundary conditions** (`plm_data/boundary_conditions/`) — Boundary-condition metadata is registry-backed. Operator subpackages declare value requirements, supported field shapes, pair requirements, and operator parameter ranges; family subpackages declare reusable domain-facing recipes such as all-Dirichlet, periodic axes, open channels, and obstacle drives. Preset specs import their operator declarations from this package, while domain specs list allowed family names and validate them against domain boundary roles.
- **Initial conditions** (`plm_data/initial_conditions/`) — Initial-condition metadata is registry-backed. Each family lives under `families/<name>/spec.py` and declares parameters, supported dimensions, vector field-level support, and seed requirements. Config validation reads allowed IC names from this registry, while domain specs validate their allowed IC families against the same metadata.

...
class TestHeterogeneousGiererMeinhardtPDE:
    ...
    def test_short_simulation(self):
        ...
        result, config = run_short_simulation("heterogeneous-gierer-meinhardt", "biology")
    ...
    def test_dimension_support(self, ndim: int):
        """Test Heterogeneous Gierer-Meinhardt works in supported dimensions.

        Note: This PDE uses a custom initial condition that assumes 2D+ grids
        (for the X coordinate field), so we only test 2D and 3D here.
        """
        np.random.seed(42)
        preset = get_pde_preset("heterogeneous-gierer-meinhardt")

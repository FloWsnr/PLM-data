    ...
    def create_initial_state(
        ...
        **kwargs,
        ...
        if ic_type in ("default", "kink"):
            ...
            if randomize:
                ic_params["x0"] = rng.uniform(x_bounds[0] + 0.2 * Lx, x_bounds[0] + 0.8 * Lx)

            x0 = ic_params.get("x0")
            if x0 is None:
                raise ValueError("sine-gordon kink requires x0")
            ...
            x0 = ic_params.get("x0")
            if x0 is None:
                raise ValueError("sine-gordon antikink requires x0")
            width = ic_params.get("width", Lx * 0.05)
            velocity = ic_params.get("velocity", 0.0)
            ...
            if randomize:
                ic_params["x0_kink"] = rng.uniform(x_bounds[0] + 0.15 * Lx, x_bounds[0] + 0.4 * Lx)
                ic_params["x0_antikink"] = rng.uniform(x_bounds[0] + 0.6 * Lx, x_bounds[0] + 0.85 * Lx)

            x0_kink = ic_params.get("x0_kink")
            x0_antikink = ic_params.get("x0_antikink")
            if x0_kink is None or x0_antikink is None:
                raise ValueError("sine-gordon kink-antikink requires x0_kink and x0_antikink")
            width = ic_params.get("width", Lx * 0.05)
            ...
            x0 = ic_params.get("x0")
            if x0 is None:
                raise ValueError("sine-gordon breather requires x0")
            ...
            if randomize:
                ic_params["x_center"] = rng.uniform(x_bounds[0] + 0.2 * Lx, x_bounds[0] + 0.8 * Lx)
                ic_params["y_center"] = rng.uniform(y_bounds[0] + 0.2 * Ly, y_bounds[0] + 0.8 * Ly)

            x_center = ic_params.get("x_center")
            y_center = ic_params.get("y_center")
            if x_center is None or y_center is None:
                raise ValueError("sine-gordon ring requires x_center and y_center")

        ...
        **kwargs,
        ...
        if ic_type in ("kdv-default", "default", "soliton"):
            ...
            if randomize:
                ...
                ic_params["x0"] = rng.uniform(x_bounds[0], x_bounds[1])

            x0 = ic_params.get("x0")
            if x0 is None:
                raise ValueError("kdv soliton requires x0")
        ...
        if ic_type == "two-solitons":
            ...
            if x1 is None or x2 is None:
                raise ValueError("kdv two-solitons requires x1 and x2")
        ...
        if ic_type == "n-wave":
            ...
            x0 = ic_params.get("x0")
            if x0 is None:
                raise ValueError("kdv n-wave requires x0")
        ...
        if ic_type == "offset-soliton":
            ...
            x0 = ic_params.get("x0")
            if x0 is None:
                raise ValueError("kdv offset-soliton requires x0")
        ...
        if ic_type == "random-solitons":
            ...
            if randomize:
                if "n" not in ic_params or "k_min" not in ic_params or "k_max" not in ic_params or "margin" not in ic_params:
                    raise ValueError("kdv random-solitons random generation requires n, k_min, k_max, margin")
                rng = np.random.default_rng(ic_params.get("seed"))
                x_range = (
                    x_bounds[0] + ic_params["margin"] * Lx,
                    x_bounds[0] + (1 - ic_params["margin"]) * Lx * 0.5,
                )
                positions = [rng.uniform(x_range[0], x_range[1]) for _ in range(ic_params["n"])]
                k_values = [rng.uniform(ic_params["k_min"], ic_params["k_max"]) for _ in range(ic_params["n"])]
                ic_params["positions"] = positions
                ic_params["k_values"] = k_values

            positions = ic_params.get("positions")
            k_values = ic_params.get("k_values")
            if positions is None or k_values is None:
                raise ValueError("kdv random-solitons requires positions and k_values")

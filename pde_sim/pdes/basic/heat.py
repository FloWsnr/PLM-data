...
class HeatPDE(ScalarPDEPreset):
    ...
    def metadata(self) -> PDEMetadata:
        return PDEMetadata(
            ...
            parameters=[
                PDEParameter("D_T", "Thermal diffusivity (diffusion coefficient)"),
            ],
            num_fields=1,
            field_names=["T"],
    ...
    def metadata(self) -> PDEMetadata:
        return PDEMetadata(
            ...
            parameters=[
                ...
                PDEParameter("n", "Spatial mode number in x direction"),
                PDEParameter("m", "Spatial mode number in y direction"),
            ],
            num_fields=1,
            field_names=["T"],
...
class InhomogeneousDiffusionHeatPDE(ScalarPDEPreset):
    ...
    def metadata(self) -> PDEMetadata:
        return PDEMetadata(
            ...
            parameters=[
                PDEParameter("D", "Base diffusion coefficient"),
                PDEParameter("E", "Modulation amplitude (0 to 1)"),
                PDEParameter("n", "Number of radial oscillations"),
            ],
            num_fields=1,
            field_names=["T"],
...
class BlobDiffusionHeatPDE(ScalarPDEPreset):
    ...
    def metadata(self) -> PDEMetadata:
        return PDEMetadata(
            ...
            equations={"T": "div(g(x,y) * grad(T))"},
            parameters=[
                PDEParameter("D_min", "Minimum diffusion coefficient (background)"),
                PDEParameter("D_max", "Maximum diffusion coefficient (at blob centers)"),
                PDEParameter("n_blobs", "Number of diffusion blobs"),
                PDEParameter("sigma", "Blob size (fraction of domain)"),
                PDEParameter("seed", "Random seed for blob placement"),
            ],
            num_fields=1,
            field_names=["T"],
            reference="https://visualpde.com/basic-pdes/inhomogeneous-diffusion",

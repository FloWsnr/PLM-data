...
1. **Presets** (`plm_data/presets/`) — Each PDE is a self-contained class registered via `@register_preset("name")`. Presets expose:
   ...
   Family-specific helpers can live alongside presets when multiple presets share a backend or discretization; `plm_data/presets/fluids/_openfoam.py` is the shared OpenFOAM fluid backend.

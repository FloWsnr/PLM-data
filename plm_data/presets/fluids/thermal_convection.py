...
from plm_data.presets.fluids.navier_stokes import (
    _domain_average,
    _polynomial_degree,
    _positive_parameter,
    _space_num_dofs,
...
def _vertical_unit_vector(gdim: int):
    ...
    components[1 if gdim == 2 else 2] = ufl.as_ufl(1.0)
...
class _ThermalConvectionProblem(TransientLinearProblem):
    ...
    def _periodic_vectors(self) -> set[frozenset[str]]:
        velocity_pairs = self.config.boundary_field("velocity").periodic_pair_keys()
        temperature_pairs = self.config.boundary_field(
            "temperature"
        ).periodic_pair_keys()
    ...
    def runtime_health_metrics(self, context: str) -> dict[str, float]:
        ...
        for field_name, values in (
            ...
            if not np.all(np.isfinite(values)):
                raise ValueError(
                    f"Preset '{self.spec.name}' produced non-finite {field_name} "

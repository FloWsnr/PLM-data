...
from plm_data.presets.boundary_validation import validate_vector_standard_boundary_field
...
def _positive_parameter(
    parameters: dict[str, float], name: str, preset_name: str
) -> float:
    value = float(parameters[name])
    if value <= 0.0:
        raise ValueError(
            f"Preset '{preset_name}' requires parameter '{name}' > 0. Got {value}."
        )
    return value


def _polynomial_degree(parameters: dict[str, float], preset_name: str) -> int:
    raw_degree = parameters["k"]
    degree = int(raw_degree)
    if float(degree) != float(raw_degree) or degree < 1:
        raise ValueError(
            f"Preset '{preset_name}' requires integer parameter 'k' >= 1. "
            f"Got {raw_degree}."
        )
    return degree
...
class _NavierStokesProblem(TransientLinearProblem):
    ...
    def runtime_health_metrics(self, context: str) -> dict[str, float]:
        ...
        if not np.all(np.isfinite(velocity_values)):
            raise ValueError(
                f"Preset '{self.spec.name}' produced non-finite velocity values "
                f"during {context}."
            )
        if not np.all(np.isfinite(pressure_values)):
            raise ValueError(
                f"Preset '{self.spec.name}' produced non-finite pressure values "
                f"during {context}."
            )
        local_velocity_max = (

"""Gierer-Meinhardt reaction-diffusion preset."""
...
_GIERER_MEINHARDT_SPEC = PresetSpec(
    name="gierer_meinhardt",
    category="biology",
    description=(
        "Gierer-Meinhardt activator-inhibitor system for Turing pattern formation. "
        "Classical morphogenesis model producing spot and stripe patterns. "
        "Turing instability requires Dh/Da >> 1."
    ),
    equations={
...
class _GiererMeinhardtProblem(TransientLinearProblem):
    ...
    def setup(self) -> None:
        ...
        # a-equation: (a_next - a_n)/dt = Da*laplacian(a_next) - mu_a*a_next + rho_a*a_n^2/h_n + sigma_a
        # Diffusion and linear decay treated implicitly
        lhs_a = (
        ...
        # h-equation: tau*(h_next - h_n)/dt = Dh*laplacian(h_next) - mu_h*h_next + rho_h*a_n^2 + sigma_h
        # Diffusion and linear decay treated implicitly; tau scales the time derivative
        lhs_h = (
...
@register_preset("gierer_meinhardt")
class GiererMeinhardtPreset(PDEPreset):
    @property

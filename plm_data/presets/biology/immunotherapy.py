"""Cancer immunotherapy reaction-diffusion preset.

Three-species model coupling effector immune cells (u), cancer cells (v),
and IL-2 cytokine (w) with Turing-pattern-forming dynamics relevant to
treatment resistance.  Based on arxiv:2503.20909.
"""
...
_IMMUNOTHERAPY_SPEC = PresetSpec(
    name="immunotherapy",
    category="biology",
    description=(
        "Three-species cancer immunotherapy model coupling effector cells (u), "
        "cancer cells (v), and IL-2 cytokine (w). Exhibits Turing instability "
        "when immune cell diffusion greatly exceeds cancer cell diffusion, "
        "leading to spatial patterns that promote tumour persistence under "
        "treatment."
    ),
    equations={
        "u": (
        ...
        "w": (
            "dw/dt = Dw*laplacian(w) + rho_w*u*v/(gamma_w+v) - mu_w*w + sigma_w + Kw*t"
        ),
    },
    parameters=[
        ...
        PDEParameter("gamma_v", "Half-saturation cancer density for immune killing"),
        PDEParameter("rho_w", "IL-2 production rate from effector-cancer interaction"),
        PDEParameter("gamma_w", "Half-saturation cancer density for IL-2 production"),
        PDEParameter("mu_w", "Natural degradation rate of IL-2"),
        PDEParameter("sigma_u", "Constant (basal) effector cell infusion rate"),
        PDEParameter("Ku", "Linear-in-time effector cell treatment rate"),
        PDEParameter("sigma_w", "Constant (basal) IL-2 infusion rate"),
...
class _ImmunotherapyProblem(TransientLinearProblem):
    ...
    def setup(self) -> None:
        ...
        # Time constant for ramped treatment terms (Ku*t, Kw*t).
        ...
        # --- u-equation (effector cells) ---
        # Implicit: diffusion + linear decay (mu_u*u).
        # Explicit: recruitment + Holling-II proliferation + treatment.
        a_u = (
        ...
        # --- v-equation (cancer cells) ---
        # Implicit: diffusion only.
        # Explicit: logistic growth + immune-mediated killing.
        a_v = (
        ...
        # --- w-equation (IL-2 cytokine) ---
        # Implicit: diffusion + linear decay (mu_w*w, stiff term).
        # Explicit: production from effector-cancer interaction + treatment.
        a_w = (
...
@register_preset("immunotherapy")
class ImmunotherapyPreset(PDEPreset):
    @property

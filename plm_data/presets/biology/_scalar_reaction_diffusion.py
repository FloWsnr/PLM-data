"""Shared scalar reaction-diffusion helper for biology presets."""
...
from plm_data.presets.base import TransientNonlinearProblem
...
def build_scalar_reaction_diffusion_spec(
    *,
    ...
    """Build a standard scalar reaction-diffusion preset spec."""
    return PresetSpec(
        name=name,
        category="biology",
...
class ScalarReactionDiffusionProblem(TransientNonlinearProblem, ABC):
    ...
    def setup(self) -> None:
        ...
        if velocity is None:
            raise ValueError(
                f"Preset '{self.spec.name}' coefficient 'velocity' cannot use a "
                "custom expression"
            )

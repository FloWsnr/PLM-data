"""Shared preset-level boundary-condition validation helpers."""
...
def validate_boundary_field_structure(
    *,
    ...
    if actual_sides != expected_sides:
        raise ValueError(
            f"Preset '{preset_name}' boundary field '{field_name}' must configure "
    ...
    for side_name, entries in boundary_field.sides.items():
        if len(entries) != expected_entries_per_side:
            raise ValueError(
                f"Preset '{preset_name}' boundary field '{field_name}' side "
        ...
        for entry in entries:
            ...
            if entry.type == "periodic":
                if entry.pair_with is None:
                    raise ValueError(
                        f"Preset '{preset_name}' boundary field '{field_name}' "

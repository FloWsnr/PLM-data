...
_KS_SPEC = PresetSpec(
    ...
    equations={
        "u": (
            ...
            "+ anti_diffusion*v - (nonlinear_strength/2)|grad(u)|^2 "
            "- damping*u"
        ),
        "v": "v + laplacian(u) = 0",
    },
    parameters=[
        ...
        PDEParameter(
            "hyperdiffusion",
            "Coefficient on the stabilizing fourth-order term.",
        ),
        PDEParameter(
            "anti_diffusion",
            "Coefficient on the destabilizing negative-diffusion term.",
        ),
        PDEParameter(
            "nonlinear_strength",
            "Coefficient on the nonlinear gradient-saturation term.",
        ),
        PDEParameter(
            "damping",
            "Linear damping used to temper or quench the chaotic state.",
        ),
...
class _KuramotoSivashinskyProblem(TransientNonlinearProblem):
    ...
    def setup(self) -> None:
        ...
        # Evolution equation with optional advection, linear damping, and tunable
        # stabilizing/destabilizing KS terms.
        F0 = (

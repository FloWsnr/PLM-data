"""Superlattice reaction-diffusion preset."""
...
_SUPERLATTICE_SPEC = PresetSpec(
    name="superlattice",
    category="physics",
    description=(
        "Coupled Brusselator and Lengyel-Epstein reaction-diffusion systems "
        "for superlattice pattern formation."
    ),
    equations={
        "u_1": (
            "du_1/dt = D_u1 * laplacian(u_1) + a - (b+1)*u_1 + u_1^2*v_1 "
            "+ alpha*u_1*u_2*(u_2-u_1)"
        ),
        "v_1": "dv_1/dt = D_v1 * laplacian(v_1) + b*u_1 - u_1^2*v_1",
        "u_2": (
            "du_2/dt = D_u2 * laplacian(u_2) + c - u_2 - 4*u_2*v_2/(1+u_2^2) "
...
class _SuperlatticeProblem(TransientLinearProblem):
    ...
    def setup(self) -> None:
        ...
        forms = {
            "u_1": (u_1_trial, u_1_test, a_u_1, L_u_1, "plm_superlattice_u_1_"),
            "v_1": (v_1_trial, v_1_test, a_v_1, L_v_1, "plm_superlattice_v_1_"),
            "u_2": (u_2_trial, u_2_test, a_u_2, L_u_2, "plm_superlattice_u_2_"),
            "v_2": (v_2_trial, v_2_test, a_v_2, L_v_2, "plm_superlattice_v_2_"),
        }
...
@register_preset("superlattice")
class SuperlatticePreset(PDEPreset):
    @property

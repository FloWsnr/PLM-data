"""Schnakenberg reaction-diffusion preset."""
...
_SCHNAKENBERG_SPEC = PresetSpec(
    name="schnakenberg",
    category="physics",
    description=(
        "Schnakenberg reaction-diffusion system for Turing pattern formation. "
        "Classical two-species activator-inhibitor model with kinetic parameters a and b. "
        "Turing instability requires Dv/Du >> 1."
    ),
    equations={
...
class _SchnakenbergProblem(TransientLinearProblem):
    ...
    def setup(self) -> None:
        ...
        # u-equation: (u_h - u_n)/dt = Du*laplacian(u_h) - u_h + a + u_n^2*v_n
        # Linear damping (-u) treated implicitly for stability
        a_u = (
        ...
        # v-equation: (v_h - v_n)/dt = Dv*laplacian(v_h) + b - u_n^2*v_n
...
@register_preset("schnakenberg")
class SchnakenbergPreset(PDEPreset):
    @property

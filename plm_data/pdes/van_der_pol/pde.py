"""Diffusively coupled Van der Pol oscillator PDE."""
...
_VAN_DER_POL_SPEC = PDESpec(
    ...
    parameters=[
        PDEParameter("Du", "Diffusion coefficient of displacement u"),
        PDEParameter("Dv", "Diffusion coefficient of velocity v"),
        PDEParameter("mu", "Nonlinear damping coefficient"),
    ],
    inputs={
        "u": InputSpec(
            name="u",
...
class _VanDerPolProblem(TransientLinearProblem):
    ...
    def setup(self) -> None:
        ...
        mu = params["mu"]
        if min(Du, Dv, mu) <= 0.0:
            raise ValueError("Van der Pol parameters Du, Dv, and mu must be positive.")
        ...
        v_rhs = mu * (1.0 - self.u_n**2) * self.v_n - self.u_n
        lhs_v = (

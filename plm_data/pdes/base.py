"""Base contracts for PDEs and reusable problem engines."""
...
from plm_data.core.runtime_config import BoundaryFieldConfig, SimulationConfig
...
class ProblemInstance(ABC):
    ...
    def __init__(self, spec: PDESpec, config: SimulationConfig):
        self.spec = spec
        self.config = config
    ...
    def _validate_solver_strategy(self) -> None:
        """Validate that the configured strategy is supported by the PDE."""
        supported = self.supported_solver_strategies
        if supported and self.config.solver.strategy not in supported:
            raise ValueError(
                f"PDE '{self.spec.name}' does not support solver strategy "
    ...
    def validate_boundary_conditions(self, domain_geom: DomainGeometry) -> None:
        """Validate boundary-condition semantics for this PDE."""
...
class _TransientProblemBase(ProblemInstance):
    ...
    def run(self, output: "FrameWriter") -> RunResult:
        ...
        if self.config.time is None:
            raise ValueError(f"PDE '{self.spec.name}' requires a time section")
...
class CustomProblem(ProblemInstance):
    """Escape hatch for PDE implementations that need full control."""

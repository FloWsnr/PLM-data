...
class _CahnHilliardProblem(TransientNonlinearProblem):
    ...
    def validate_boundary_conditions(self, domain_geom):
        ...
        for side_name, entries in boundary_field.sides.items():
            ...
            if (
                ...
                raise ValueError(
                    "PDE 'cahn_hilliard' supports only homogeneous Neumann "
                    f"natural no-flux boundaries. Side '{side_name}' got {value!r}."
                )
    ...
    def setup(self) -> None:
        ...
        if min(lmbda, barrier_height, mobility) <= 0.0:
            raise ValueError(
                "Cahn-Hilliard lmbda, barrier_height, and mobility must be positive."
            )
        if not 0.0 <= theta <= 1.0:
            raise ValueError("Cahn-Hilliard theta must be between 0 and 1.")

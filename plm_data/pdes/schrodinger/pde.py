"""Linear time-dependent Schrodinger equation via a real/imaginary split."""
...
class _SchrodingerProblem(TransientLinearProblem):
    ...
    def setup(self) -> None:
        ...
        theta = params["theta"]
        if D <= 0.0:
            raise ValueError("Schrodinger parameter 'D' must be positive.")
        if not 0.5 <= theta <= 1.0:
            raise ValueError(
                f"Schrodinger parameter 'theta' must lie in [0.5, 1.0]. Got {theta}."
            )

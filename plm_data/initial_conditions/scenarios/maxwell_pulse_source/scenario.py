"""Maxwell pulse source initial-condition scenario."""

from typing import TYPE_CHECKING

from plm_data.core.runtime_config import (
    DomainConfig,
    FieldExpressionConfig,
    InputConfig,
)
from plm_data.initial_conditions.scenarios.base import (
    InitialConditionScenario,
    InitialConditionScenarioSpec,
    register_initial_condition_scenario,
)

if TYPE_CHECKING:
    from plm_data.sampling.context import SamplingContext


def _uniform(
    context: "SamplingContext",
    name: str,
    minimum: float,
    maximum: float,
) -> float:
    value = float(context.child(name).rng.uniform(minimum, maximum))
    context.values[name] = value
    return value


def _expr(expr_type: str, **params) -> FieldExpressionConfig:
    return FieldExpressionConfig(type=expr_type, params=params)


def _vector_expr(**components: FieldExpressionConfig) -> FieldExpressionConfig:
    return FieldExpressionConfig(components=components)


def _build(
    context: "SamplingContext",
    domain: DomainConfig,
    parameters: dict[str, float],
) -> dict[str, InputConfig]:
    if domain.type != "rectangle":
        raise ValueError(
            "maxwell_pulse_source supports only rectangle domains. "
            f"Got {domain.type!r}."
        )

    length, height = domain.params["size"]
    scale = min(float(length), float(height))
    center = [
        _uniform(context, "maxwell.source.center_x", 0.35 * length, 0.65 * length),
        _uniform(context, "maxwell.source.center_y", 0.35 * height, 0.65 * height),
    ]
    sigma = _uniform(context, "maxwell.source.sigma", 0.08 * scale, 0.14 * scale)
    source = _vector_expr(
        x=_expr("gaussian_bump", amplitude=1.0, sigma=sigma, center=center),
        y=_expr(
            "gaussian_bump",
            amplitude=_uniform(context, "maxwell.source.y_amp", 0.8, 1.2),
            sigma=sigma,
            center=center,
        ),
    )
    return {
        "electric_field": InputConfig(
            source=source,
            initial_condition=FieldExpressionConfig(type="zero"),
        )
    }


SCENARIO = register_initial_condition_scenario(
    InitialConditionScenario(
        spec=InitialConditionScenarioSpec(
            name="maxwell_pulse_source",
            description="Zero initial electric field with localized pulsed source.",
            supported_dimensions=(2,),
            supported_pdes=("maxwell_pulse",),
            supported_domains=("rectangle",),
            configured_inputs=("electric_field",),
            field_shapes=("vector",),
            operators=("zero", "gaussian_bump", "constant"),
            coordinate_regions=("interior", "center"),
        ),
        build=_build,
    )
)

"""Cyclic competition mixed initial-condition scenario."""

from typing import TYPE_CHECKING

from plm_data.core.runtime_config import (
    DomainConfig,
    FieldExpressionConfig,
    InputConfig,
)
from plm_data.initial_conditions.scenarios.base import (
    InitialConditionScenario,
    InitialConditionScenarioSpec,
    register_initial_condition_scenario,
)

if TYPE_CHECKING:
    from plm_data.sampling.context import SamplingContext


def _uniform(
    context: "SamplingContext",
    name: str,
    minimum: float,
    maximum: float,
) -> float:
    value = float(context.child(name).rng.uniform(minimum, maximum))
    context.values[name] = value
    return value


def _expr(expr_type: str, **params) -> FieldExpressionConfig:
    return FieldExpressionConfig(type=expr_type, params=params)


def _build(
    context: "SamplingContext",
    domain: DomainConfig,
    parameters: dict[str, float],
) -> dict[str, InputConfig]:
    if domain.type != "rectangle":
        raise ValueError(
            "cyclic_competition_mixed currently supports only rectangle domains. "
            f"Got {domain.type!r}."
        )

    coexistence = 1.0 / (1.0 + parameters["a"] + parameters["b"])
    noise = _uniform(context, "cyclic.ic.noise", 0.012, 0.035)
    u_shift = _uniform(context, "cyclic.ic.u_shift", -0.025, 0.025)
    v_shift = _uniform(context, "cyclic.ic.v_shift", -0.025, 0.025)
    return {
        "u": InputConfig(
            initial_condition=_expr(
                "gaussian_noise",
                mean=max(0.05, coexistence + u_shift),
                std=noise,
            )
        ),
        "v": InputConfig(
            initial_condition=_expr(
                "gaussian_noise",
                mean=max(0.05, coexistence + v_shift),
                std=noise,
            )
        ),
        "w": InputConfig(
            initial_condition=_expr(
                "gaussian_noise",
                mean=max(0.05, coexistence - u_shift - v_shift),
                std=noise,
            )
        ),
    }


SCENARIO = register_initial_condition_scenario(
    InitialConditionScenario(
        spec=InitialConditionScenarioSpec(
            name="cyclic_competition_mixed",
            description="Noisy perturbation around cyclic coexistence.",
            supported_dimensions=(2,),
            supported_pdes=("cyclic_competition",),
            supported_domains=("rectangle",),
            configured_inputs=("u", "v", "w"),
            field_shapes=("scalar",),
            operators=("gaussian_noise",),
            coordinate_regions=("interior",),
        ),
        build=_build,
    )
)
